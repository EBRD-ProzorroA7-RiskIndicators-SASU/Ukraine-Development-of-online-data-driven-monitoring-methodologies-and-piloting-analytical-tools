...
public class ProcedureFilterService extends BaseDataExtractor {
    ...
    public ProcedureFilterService(RestHighLevelClient client,
    ...
    public FilteringDTO filter(FilterQuery filterQuery) {
        ...
        if (isEmpty(proceduresWrapper.getProcedures())) {
    ...
    public List<TenderIndicatorsCommonInfo> getForExport(List<String> tenderIds) {
        ...
        SearchResponse searchResponse = getSearchResponse(searchRequest, client);
        List<TenderIndicatorsCommonInfo> result = responseToEntities(searchResponse);

        calcScoreRank(result);
    ...
    List<KeyValueObject> getFilterData(FilterQuery filterQuery) {
        ...
        searchSourceBuilder.sort(DATE_PUBLISHED.value(), SortOrder.DESC);
        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.TEMP.value())
    ...
    private void addSortParameters(SearchSourceBuilder searchSourceBuilder, FilterQuery filterQuery) {
        if (nonNull(filterQuery.getSortField())) {
            ...
            if (nonNull(filterQuery.getInQueue()) && filterQuery.getInQueue()) {
                searchSourceBuilder.sort("materialityScore", SortOrder.DESC);
            } else {
                searchSourceBuilder.sort("datePublished", SortOrder.DESC);
                searchSourceBuilder.sort("tenderId.keyword", SortOrder.ASC);
            }
        }
    }
    ...
    private String fetchSortField(String sortField) {
        ...
        } else if (Mapping.SORT_FIELDS_WITHOUT_KEYWORD.contains(sortField)) {
            return sortField;
        }
    ...
    BoolQueryBuilder getBoolQueryWithFilters(FilterQuery filterQuery) {
        ...
        if (isNull(filterQuery.getInQueue()) || !filterQuery.getInQueue()) {
    ...
    private void addAllFilters(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (StringUtils.isNotEmpty(filterQuery.getProcedureId())) {
            ...
            filterByInQueue(filterQuery, mainQuery);
            filterByHasPriorityStatus(filterQuery, mainQuery);
            filterByHasChecklist(filterQuery, mainQuery);
            filterByFeedbackStatuses(filterQuery, mainQuery);
        }
    }
    ...
    private AvailableFilters getAvailableFilters(SearchResponse searchResponse) {
        ...
        availableFilters.setProcedureTypes(parseStringTermsAggregationResultsToKeyValueList(searchResponse, Aggregations.PROCEDURE_TYPE.value()), true);
        ...
        availableFilters.setProcuringEntityKind(parseStringTermsAggregationResultsToKeyValueList(searchResponse, Aggregations.PROCURING_ENTITY_KIND.value()), true);
        ...
        availableFilters.setQueuePriority(new ArrayList<>(Mapping.QUEUE_PRIORITY.values()));
        availableFilters.setFeedbackStatuses(parseStringTermsAggregationResultsToKeyValueList(searchResponse, Aggregations.FEEDBACK_STATUSES.value()));

        availableFilters.setTenderScoreRank(getTenderScoreRank(parseDoubleTermsAggregationResultsToKeyValueList(searchResponse, Aggregations.TENDER_RISK_SCORE.value())));

        return availableFilters;
    }

    private List<KeyValueObject> getTenderScoreRank(List<KeyValueObject> scores) {
        ConfigurationDomain configuration = configurationService.getConfiguration();

        List<KeyValueObject> tenderRank = new ArrayList<>();

        AtomicLong lowCount = new AtomicLong();
        scores.stream()
                .filter(item -> (double) item.getKey() < configuration.getBucketRiskGroupMediumLeft())
                .forEach(item -> lowCount.addAndGet((Long) item.getValue()));
        ...
        scores.stream()
                .filter(item -> (double) item.getKey() >= configuration.getBucketRiskGroupMediumLeft() &&
                        (double) item.getKey() <= configuration.getBucketRiskGroupMediumRight())
                .forEach(item -> mediumCount.addAndGet((Long) item.getValue()));
        ...
        scores.stream()
                .filter(item -> (double) item.getKey() > configuration.getBucketRiskGroupMediumRight())
                .forEach(item -> highCount.addAndGet((Long) item.getValue()));
    ...
    private long parseInternalFilterAggregationDocCount(SearchResponse searchResponse, String aggregationName) {
        return ((ParsedFilter) searchResponse
                .getAggregations()
    ...
    private List<KeyValueObject> parseNameAndCodeAggregationResultsToKeyValueList(SearchResponse searchResponse, String aggregationName) {
        return ((Terms) searchResponse
                ...
                .map(bucket -> new KeyValueObject(
                        ...
                        ((Terms) bucket.getAggregations().get(Aggregations.CODE_SUB_AGGREGATION.value()))
    ...
    private List<KeyValueObject> parseDoubleTermsAggregationResultsToKeyValueList(SearchResponse searchResponse, String aggregationName) {
        ParsedDoubleTerms doubleTerms = searchResponse
                .getAggregations()
                .get(aggregationName);
    ...
    private void addAggregationForAvailableFilters(SearchSourceBuilder searchSourceBuilder) {
        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.REGION.value())
                .field(ProcedureProperty.REGION_KEYWORD.value()).size(25));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.MONITORING_OFFICE.value())
                .field(MONITORING_OFFICE_KEYWORD.value()).size(50));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.MONITORING_STATUS.value())
                .field(ProcedureProperty.MONITORING_STATUS_KEYWORD.value()).size(15));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.GSW.value())
                .field(ProcedureProperty.GSW_KEYWORD.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.TENDER_STATUS.value())
                .field(ProcedureProperty.TENDER_STATUS_KEYWORD.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.PROCEDURE_TYPE.value())
                .field(ProcedureProperty.PROCEDURE_TYPE_KEYWORD.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.COMPLAINS.value())
                .field(ProcedureProperty.HAS_COMPLAINS.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.MONITORING_APPEAL.value())
                .field(ProcedureProperty.MONITORING_APPEAL.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.MONITORING_CAUSE.value())
                .field(ProcedureProperty.MONITORING_CAUSE.value()));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.INDICATORS_WITH_RISK.value())
                .field(ProcedureProperty.INDICATORS_WITH_RISK_KEYWORD.value()).size(100));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.PROCURING_ENTITY_KIND.value())
                .field(ProcedureProperty.PROCURING_ENTITY_KIND_KEYWORD.value()).size(10));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.PROCURING_ENTITY_NAME.value())
        ...
        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.CPV_NAME.value())
                .field(ProcedureProperty.CPV_NAME_KEYWORD.value()).size(10).subAggregation(
                        AggregationBuilders.terms(Aggregations.CODE_SUB_AGGREGATION.value())
                                .field(CPV_KEYWORD.value())
                ));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.CPV2_NAME.value())
                .field(ProcedureProperty.CPV2_NAME_KEYWORD.value()).size(10).subAggregation(
                        AggregationBuilders.terms(Aggregations.CODE_SUB_AGGREGATION.value())
                                .field(CPV2_KEYWORD.value())
                ));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.CURRENCY.value())
                .field(ProcedureProperty.CURRENCY_KEYWORD.value()).size(101));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.TENDER_RISK_SCORE.value())
                .field(TENDER_RISK_SCORE.value()).size(100));

        searchSourceBuilder.aggregation(AggregationBuilders.filter(WITH_RISK.value(), QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value()).gt(0)));
        searchSourceBuilder.aggregation(AggregationBuilders.filter(WITHOUT_RISK.value(), QueryBuilders.boolQuery().mustNot(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value()).gt(0))));

        searchSourceBuilder.aggregation(AggregationBuilders.filter(WITH_RISK_HAS_PRIORITY.value(), QueryBuilders.boolQuery()
                .must(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value()).gt(0))
                .must(QueryBuilders.termQuery(HAS_PRIORITY_STATUS.value(), true))));

        searchSourceBuilder.aggregation(AggregationBuilders.filter(WITH_RISK_NO_PRIORITY.value(), QueryBuilders.boolQuery()
                .must(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value()).gt(0))
                .mustNot(QueryBuilders.termQuery(HAS_PRIORITY_STATUS.value(), true))));

        searchSourceBuilder.aggregation(AggregationBuilders.terms(Aggregations.FEEDBACK_STATUSES.value())
                .field(FEEDBACK_STATUSES_KEYWORD.value()).size(10));
    }
    ...
    private void filterByGsw(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (StringUtils.isNotEmpty(filterQuery.getGsw())) {
            if (Mapping.WORKS_GSW.equals(filterQuery.getGsw())) {
                mainQuery.must(QueryBuilders.termsQuery(
                        GSW_KEYWORD.value(),
                        Mapping.CUSTOM_WORKS_GSW,
                        Mapping.WORKS_GSW));
            } else {
    ...
    private void filterProcuringEntityType(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (StringUtils.isNotEmpty(filterQuery.getProcuringEntityKind())) {
            ...
            for (String kind : filterQuery.getProcuringEntityKind().split(",")) {
                if (StringUtils.isNotEmpty(kind)) {
                    boolQuery.should(QueryBuilders.termQuery(PROCURING_ENTITY_KIND_KEYWORD.value(), kind));
                }
            }
    ...
    private void filterByMonitoringStatus(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        ...
        if (!isEmpty(filterQuery.getMonitoringStatus())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByInQueue(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (filterQuery.getInQueue() != null) {
            mainQuery.must(QueryBuilders.termQuery(IN_QUEUE.value(), filterQuery.getInQueue()));
        }
    }

    private void filterByHasPriorityStatus(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (filterQuery.getHasPriorityStatus() != null) {
            mainQuery.must(QueryBuilders.termQuery(HAS_PRIORITY_STATUS.value(), filterQuery.getHasPriorityStatus()));
        }
    }

    private void filterByHasChecklist(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (nonNull(filterQuery.getHasChecklist())) {
            if (filterQuery.getHasChecklist()) {
                mainQuery.must(
                        QueryBuilders.termsQuery(
                                PROCEDURE_LOG_TYPE.value(),
                                Mapping.PROCEDURE_LOG_TYPE_CHECKLIST_SOLUTIONS.entrySet().stream()
    ...
    private void filterByRegion(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getRegions())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByMonitoringOffice(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getMonitoringOffices())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByMonitoringCause(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getMonitoringCause())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByProcuringEntities(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getProcuringEntities())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByTenderStatus(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getTenderStatuses())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByCurrency(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getCurrency())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByCpv2Name(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getCpv2Names())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByCpvName(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getCpvNames())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
    ...
    private void filterByProcedureType(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getProcedureTypes())) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
            for (String procedureType : filterQuery.getProcedureTypes()) {
                if (StringUtils.isNotEmpty(procedureType)) {
                    ...
                    if ("negotiation".equalsIgnoreCase(procedureType)) {
                        boolQuery.should(QueryBuilders.matchQuery(ProcedureProperty.PROCEDURE_TYPE.value(), "negotiation.quick"));
                    }
                }
            }
            boolQuery.minimumShouldMatch(1);
    ...
    private void filterRiskedProcedureByRank(FilterQuery filterQuery, BoolQueryBuilder boolQuery) {
        if (filterQuery.getTenderRank() == null || isEmpty(filterQuery.getTenderRank())) {
            return;
        }
        ...
        if (filterQuery.getTenderRank().contains(TenderScoreRank.HIGH)) {
            tempQuery.should(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value())
                    .gt(configuration.getBucketRiskGroupMediumRight())
            );
        }
        if (filterQuery.getTenderRank().contains(TenderScoreRank.MEDIUM)) {
            tempQuery.should(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value())
                    .lte(configuration.getBucketRiskGroupMediumRight())
                    .gte(configuration.getBucketRiskGroupMediumLeft())
            );
        }
        if (filterQuery.getTenderRank().contains(TenderScoreRank.LOW)) {
            tempQuery.should(QueryBuilders.rangeQuery(TENDER_RISK_SCORE.value())
                    .lt(configuration.getBucketRiskGroupMediumLeft())
                    .gte(0.0)
            );
        }
    ...
    private void filterByRiskedIndicators(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getRiskedIndicators())) {
    ...
    private void filterByFeedbackStatuses(FilterQuery filterQuery, BoolQueryBuilder mainQuery) {
        if (!isEmpty(filterQuery.getFeedbackStatuses())) {
            List<String> feedbackStatuses = filterQuery.getFeedbackStatuses();
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
            for (String feedbackStatus : feedbackStatuses) {
                boolQuery.should(QueryBuilders.termQuery(FEEDBACK_STATUSES_KEYWORD.value(), feedbackStatus));
            }
            boolQuery.minimumShouldMatch(1);
            mainQuery.must(boolQuery);
        }
    ...
    public String getLastTenderDate() {
        SearchRequest searchRequest = getRequestBuilder();
        ...
        return results.get(0).getDatePublished();
    }
}
