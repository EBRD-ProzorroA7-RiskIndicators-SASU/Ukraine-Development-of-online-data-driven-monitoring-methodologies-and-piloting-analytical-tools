...
public class Mapping {
    ...
    public static final Map<String, KeyValueObject> PROCEDURE_LOG_TYPES;
    ...
    public static final Map<String, KeyValueObject> QUEUE_PRIORITY;
    public static final Map<String, KeyValueObject> APPEAL;
    public static final Map<String, KeyValueObject> PROCURING_ENTITY_KIND;
    public static final Map<String, KeyValueObject> FEEDBACK_STATUS;
    ...
    public static final LocalDate CUSTOM_BELOW_THRESHOLD_MODIFY_DATE = LocalDate.parse("2020-04-18");
    public static final String CUSTOM_BELOW_THRESHOLD_TYPE = "Спрощена закупівля";

    public static final String CUSTOM_WORKS_GSW = "Works";
    public static final String WORKS_GSW = "works";

    //contains boolean values that manage checklist statuses (hasChecklist, availableForChecklist)
    public static Map<String, Boolean[]> PROCEDURE_LOG_TYPE_CHECKLIST_SOLUTIONS;
    ...
    static {
        ...
        PROCEDURE_LOG_TYPES = initMapping("procedureLogTypes");
        ...
        QUEUE_PRIORITY = initMapping("queuePriority");
        FEEDBACK_STATUS = initMapping("feedbackStatus");
        PROCEDURE_LOG_TYPE_CHECKLIST_SOLUTIONS = initProcedureLogTypeChecklistSolutions();
    }
    ...
    private static Map<String, Boolean[]> initProcedureLogTypeChecklistSolutions() {
        Map<String, Boolean[]> inputList = new HashMap<>(10);
        try {
            InputStream is = new ClassPathResource("mappings/procedureLogTypes.tsv").getInputStream();
            ...
            br.lines()
                    ...
                    .forEach(item -> inputList.put(item.getKey(), item.getValue()));
            br.close();
        } catch (IOException e) {
    ...
    static {
        ...
        EXPORT_FIELD_MAPPING.add(new KeyValueObject("materialityScore", "Коефіцієнт матеріальності"));
    }
    ...
    public static String extractProcedureType(String procedureType, String datePublished) {
        if (procedureType.contains("negotiation")) {
            return PROCEDURE_TYPES.get("negotiation").getValue().toString();
        } else if (procedureType.equals("belowThreshold") && LocalDate.parse(datePublished).isAfter(CUSTOM_BELOW_THRESHOLD_MODIFY_DATE)) {
            return CUSTOM_BELOW_THRESHOLD_TYPE;
        } else {
            return PROCEDURE_TYPES.get(procedureType).getValue().toString();
        }
    }

    public static String extractGswValue(String gsw) {
        if (CUSTOM_WORKS_GSW.equals(gsw)) return GSW.get(WORKS_GSW).getValue().toString();
        return GSW.get(gsw).getValue().toString();
    }

    public static KeyValueObject extractGsw(String gsw) {
        if (CUSTOM_WORKS_GSW.equals(gsw)) return GSW.get(WORKS_GSW);
        return GSW.get(gsw);
    }
}
